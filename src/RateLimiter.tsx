...
import { rateLimiterOptions, useRateLimiter } from "@tanstack/react-pacer/rate-limiter";
...
export function RateLimiterDemo() {
	// manual trigger rerender every 100ms to ensure updates on 'remaining' items
	useNow(100);
	...
	const common = useMemo(() => rateLimiterOptions({ limit, window: windowMs }), [limit, windowMs]);

	const rateLimiter = useRateLimiter(
		(_payload: number) => {
			// No-op. We display allowed/rejected from the rateLimiter's own state.
			void _payload;
		},
		{
			...common,
...
rateLimiter.maybeExecute(payload)`;
	...
	function send(n: number) {
		// Avoid side effects inside a React state updater.
		// In Strict Mode/dev, React may invoke updater functions more than once.
		for (let i = 0; i < n; i += 1) {
			attemptRef.current += 1;
			rateLimiter.maybeExecute(attemptRef.current);
		}
		setInstantCount(attemptRef.current);
	}
			...
			<div className="content">
				<div className="help">
					...
					<ul className="helpList">
						...
						<li>
							Watch <b>Remaining</b> drop to 0, then wait for the window to reset.
						</li>
						<li>
							Switch <b>Fixed</b> vs <b>Sliding</b> to see how “refill” timing changes.
						</li>
					</ul>
				</div>
					...
					<Field label="Window type">
						<div className="radioRow">
							<label className="radio">
								<input type="radio" checked={windowType === "fixed"} onChange={() => setWindowType("fixed")} />
								Fixed
							</label>
							<label className="radio">
								<input type="radio" checked={windowType === "sliding"} onChange={() => setWindowType("sliding")} />
								Sliding
							</label>
						</div>
					</Field>
				...
				<div className="help">
					...
					<ul className="helpList">
						<li>
							<b>limit</b> + <b>window</b>: the quota and the time range it applies to.
						</li>
						<li>
							<b>windowType=fixed</b>: resets in chunks; you can burst again after the reset.
						</li>
						<li>
							<b>windowType=sliding</b>: a moving window; capacity “returns” gradually over time.
						</li>
					</ul>
				</div>
				...
				<div className="stats">
					<Stat label="Status" value={<Pill>{isLimited ? "limited" : "ok"}</Pill>} />
					<Stat label="Instant clicks" value={instantCount} />
					<Stat label="Allowed" value={rateLimiter.state.executionCount} />
					...
					<Stat label="Last reject (ms)" value={lastRejectMs != null ? lastRejectMs : "—"} />
				</div>

				<details className="details">
					<summary>Advanced: full rate limiter state</summary>
					<rateLimiter.Subscribe selector={(s) => s}>{(s) => <CodeBlock value={s} />}</rateLimiter.Subscribe>
				</details>
			</div>
		</Card>
	);
